...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
  if (request.action === "startScan") {
    ...
    // Schedule periodic scanning (every 10 seconds)
...
function checkLoginAndNavigate() {
  ...
  if (!window.location.href.includes('deleted-com-domains')) {
    ...
    if (deletedDomainsLink) {
      chrome.tabs.update({ url: deletedDomainsLink.href });
    } else {
      chrome.tabs.update({ url: 'https://www.expireddomains.net/deleted-com-domains/' });
    }
    return;
  }
